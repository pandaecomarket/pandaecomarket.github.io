...
;(function($) {
  ...
  var cartData,
    ...
    orderform = '<p class="jqcart-cart-title">Контактна інформація:</p><form class="jqcart-orderform"><p><label>ПІБ:</label><input type="text" name="user_name"></p><p><label>Телефон:</label><input type="text" name="user_phone"></p><p><label>Email:</label><input type="text" name="user_mail"></p><p><label>Адреса:</label><input type="text" name="user_address"></p><p><label>Коментар:</label><textarea name="user_comment"></textarea></p><p><input type="submit" value="Відправити замовлення"><input type="reset" value="Повернутися"></p></form>';
  var opts = {
		buttons: '.add_item',
		cartLabel: 'body',
		visibleLabel: false,
  ...
  var actions = {
    ...
    openCart: function() {
      var subtotal = 0,
			cartHtml = '';
      cartData = actions.getStorage();
      orderPreview = '<p class="jqcart-cart-title">Кошик <span class="jqcart-print-order"></span></p><div class="jqcart-table-wrapper"><div class="jqcart-manage-order"><div class="jqcart-thead"><div>ID</div><div></div><div>Найменування</div><div>Вартість</div><div>Кол-во</div><div>Сума</div><div></div></div>';
      var key, sum = 0;
      for (key in cartData) {
        if (cartData.hasOwnProperty(key)) {
      ...
      orderPreview += '<div class="jqcart-subtotal">Усього: <strong>' + subtotal + '</strong> ' + opts.currency + '</div>';
			
			cartHtml = subtotal ? (orderPreview + orderform) : '<h2 class="jqcart-empty-cart">Кошик пустий</h2>';
