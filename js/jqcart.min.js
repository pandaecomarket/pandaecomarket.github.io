...
*/
(function(d){var a,k,g="",n=0,p=!1,l=d('<div class="jqcart-cart-label"><span class="jqcart-title">Оформити замовлення</span><span class="jqcart-total-cnt">0</span></div>'),h={buttons:".add_item",cartLabel:"body",visibleLabel:!1,openByAdding:!1,handler:"/",currency:"₴"},c={init:function(b){h=d.extend(h,b);a=c.getStorage();if(null!==a&&Object.keys(a).length){for(var e in a)a.hasOwnProperty(e)&&(n+=a[e].count);p=!0}l.prependTo(h.cartLabel)[p||
h.visibleLabel?"show":"hide"]().on("click",c.openCart).find(".jqcart-total-cnt").text(n);d(document).on("click",h.buttons,c.addToCart).on("click",".jqcart-layout",function(b){b.target===this&&c.hideCart()}).on("click",".jqcart-incr",c.changeAmount).on("input keyup",".jqcart-amount",c.changeAmount).on("click",".jqcart-del-item",c.delFromCart).on("submit",".jqcart-orderform",c.sendOrder).on("reset",".jqcart-orderform",c.hideCart);return!1},addToCart:function(b){b.preventDefault();
...
a[e].count+'"><span class="jqcart-incr" data-incr="1">+</span></div>';g+='<div class="jqcart-sum">'+k+" "+h.currency+"</div>";g+='<div class="jqcart-small-td"><span class="jqcart-del-item"></span></div>';g+="</div>"}g+="</div></div>";g+='<div class="jqcart-subtotal">До сплати: <strong>'+b+"</strong> "+h.currency+"</div>";b=b?g+'<p class="jqcart-cart-title">Контактна інформація:</p><form class="jqcart-orderform"><p><label>ПІБ:</label><input type="text" name="user_name"></p><p><label>Телефон:</label><input type="text" name="user_phone"></p><p><label>Email:</label><input type="text" name="user_mail"></p><p><label>Адреса:</label><input type="text" name="user_address"></p><p><p><input type="submit" value="Замовити"><input type="reset" value="Повернутись"></p></form>':
'<h2 class="jqcart-empty-cart">У кошику пусто!</h2>';d('<div class="jqcart-layout"><div class="jqcart-checkout">123</div></div>').appendTo("body").find(".jqcart-checkout").html(b)},hideCart:function(){d(".jqcart-layout").fadeOut("fast",function(){d(this).remove()});return!1},sendOrder:function(b){b.preventDefault();b=d(this);if(""===d.trim(d("[name=user_name]",b).val())||""===d.trim(d("[name=user_phone]",b).val()))return d('<p class="jqcart-error">Будь ласка, введіть Ваші дані!</p>').insertBefore(b).delay(3E3).fadeOut(),
